...
export function Modifications(props: ModificationsProps) {
  ...
  const fetchOwnEvents = useCallback(async () => {
    ...
    try {
      ...
      const list: Event[] = Array.isArray(res)
        ? res
        : (res?.items ?? res?.events ?? res?.data ?? []);
      if (mountedRef.current) setOwnEvents(list);
    } catch (e) {
      console.error(e);
      if (mountedRef.current)
        setEvError("Events konnten nicht geladen werden.");
    } finally {
  ...
  const hasExternalEventSelection =
    Array.isArray(props.selectedEvents) &&
    typeof props.setSelectedEvents === "function";

  const [ownSelectedEvents, setOwnSelectedEvents] = useState<string[]>([]);
  const selectedEvents: string[] = hasExternalEventSelection
    ? (props.selectedEvents as string[])
    : ownSelectedEvents;
  const setSelectedEvents: React.Dispatch<React.SetStateAction<string[]>> =
    hasExternalEventSelection
      ? (props.setSelectedEvents as React.Dispatch<
          React.SetStateAction<string[]>
        >)
      : setOwnSelectedEvents;

  const refreshAfterMutation = useCallback(
    (what?: "events" | "tags" | "participants") => {
  ...
  const selectedTag = useMemo(
    () =>
      selectedTags.length === 1
        ? tags.find((t) => t._id === selectedTags[0])
        : undefined,
    [selectedTags, tags],
  );
  const selectedPart = useMemo(
    () =>
      selectedParticipants.length === 1
        ? participants.find((p) => p._id === selectedParticipants[0])
        : undefined,
  ...
  const selectedEvent = useMemo(
    () =>
      selectedEvents.length === 1
        ? events.find((e) => e._id === selectedEvents[0])
        : undefined,
  ...
  useEffect(() => {
    ...
    setEvDateLocal(
      selectedEvent?.date
        ? toLocalDatetimeInputValue(new Date(selectedEvent.date))
        : "",
    );
  ...
  async function doDeleteParticipants() {
    try {
      await api("/participants", {
        method: "DELETE",
        body: JSON.stringify({ ids: selectedParticipants }),
      });
      setSelectedParticipants([]);
      showToast(
        "success",
        `${selectedParticipants.length} Teilnehmer gelöscht.`,
      );
  ...
  async function doUpdateEvent() {
    ...
    try {
      ...
      if (!LOCATION_RE.test(location)) {
        showToast(
          "error",
          "Adresse unzulässig. Erlaubt: Buchstaben/Ziffern . , - / ( ); keine führenden/abschließenden Symbole.",
        );
        return;
      }
  ...
  return (
    <div className="card">
          ...
          {tags.map((tag) => (
            <label key={tag._id} className="row">
              <input
                ...
                disabled={
                  selectedParticipants.length > 0 || selectedEvents.length > 0
                }
              />
              <span
                className="tag"
      ...
      <section className="section">
        ...
        <div className="form-grid">
          ...
          <button
            ...
            onClick={() =>
              askConfirm(
                "Ausgewählten Teilnehmer aktualisieren?",
                doUpdateParticipant,
              )
            }
          >
      ...
      <section className="section">
        ...
        <div className="list">
          {events.map((ev) => {
            const labelDate = ev.date
              ? new Date(ev.date).toLocaleString("de-DE")
              : "—";
            return (
                ...
                <input
                  ...
                  onChange={() =>
                    setSelectedEvents((prev) =>
                      prev.includes(ev._id)
                        ? prev.filter((id) => id !== ev._id)
                        : [...prev, ev._id],
                    )
                  }
                  disabled={
        ...
        <div className="form-grid form-grid-ev">
          ...
          <button
            ...
            onClick={() =>
              askConfirm("Ausgewähltes Event aktualisieren?", doUpdateEvent)
            }
          >
            Event aktualisieren
          </button>
