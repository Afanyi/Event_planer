...
export default function EventForm({
  ...
  async function create(e: React.FormEvent<HTMLFormElement>) {
    ...
    const titleInput = formEl.elements.namedItem(
      "title",
    ) as HTMLInputElement | null;
    const dateInput = formEl.elements.namedItem(
      "date",
    ) as HTMLInputElement | null;
    const streetInput = formEl.elements.namedItem(
      "street",
    ) as HTMLInputElement | null;
    const houseInput = formEl.elements.namedItem(
      "houseNumber",
    ) as HTMLInputElement | null;
    const plzInput = formEl.elements.namedItem(
      "postalCode",
    ) as HTMLInputElement | null;
    const cityInput = formEl.elements.namedItem(
      "city",
    ) as HTMLInputElement | null;
    ...
    if (title.length < 3 || title.length > 80 || !TITLE_RE.test(title)) {
      const msg =
        "Invalid title. Must be 3–80 characters, letters/numbers only, no leading/trailing symbols.";
