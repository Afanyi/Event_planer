...
describe("App (integration)", () => {
      ...
      async (url: string, opts?: RequestInit) => {
        ...
        if (url === "/events" && opts?.method === "POST") {
          ...
          const newEvent: Event = {
            ...
            tags: (body.tags || []).map(
              (id: string) => TAGS.find((t) => t._id === id)!,
            ),
            participants: (body.participants || []).map(
              (pid: string) => PARTICIPANTS.find((p) => p._id === pid)!,
            ),
          };
        ...
        if (url.includes("/tags/") || url.includes("/participants/"))
          return { ok: true };
  ...
  it("applies filters and calls /events? with correct query string", async () => {
    ...
    const toInput = root.querySelectorAll(
      'input[type="date"]',
    ...
    await waitFor(() => {
      ...
      const calls = (api as jest.Mock).mock.calls
        .map((c) => c[0])
        .filter(
          (u: string) => typeof u === "string" && u.startsWith("/events?"),
        );
