...
describe('TagList (unit)', () => {
    ...
    it('submits successfully: calls api, resets form, and calls onChanged', async () => {
        ...
        const colorInput = screen.getByPlaceholderText('#rrggbb') as HTMLInputElement;
    ...
    it('shows API error from Response.json() when response.ok === false', async () => {
        ...
        const addBtn = screen.getByRole('button', { name: 'Add' });
        const form = addBtn.closest('form')!;
        fireEvent.change(screen.getByPlaceholderText('Name'), { target: { value: 'Work' } });
        fireEvent.submit(form);

        await waitFor(() => {
            // Alert is rendered with our message
            expect(screen.getByRole('alert')).toHaveTextContent('HTTP 409: Duplicate tag');
    ...
    it('shows fallback API error when api() returns object with { error }', async () => {
        ...
        const addBtn = screen.getByRole('button', { name: 'Add' });
        const form = addBtn.closest('form')!;
        fireEvent.change(screen.getByPlaceholderText('Name'), { target: { value: 'Oops' } });
        fireEvent.submit(form);

        await waitFor(() => {
            expect(screen.getByRole('alert')).toHaveTextContent('HTTP 400: Invalid color');
            expect(mockOnChanged).not.toHaveBeenCalled();
        });
    });
    ...
    it('shows network/unknown error when api() throws', async () => {
        ...
        const addBtn = screen.getByRole('button', { name: 'Add' });
        const form = addBtn.closest('form')!;
        fireEvent.change(screen.getByPlaceholderText('Name'), { target: { value: 'TagX' } });
        fireEvent.submit(form);
