...
describe("EventList (unit)", () => {
  ...
  it("clicking an existing tag chip triggers removeTag", async () => {
    ...
    await waitFor(() => {
      expect(api).toHaveBeenCalledWith("/events/e1/tags/t1", {
        method: "DELETE",
      });
      expect(onChanged).toHaveBeenCalledTimes(1);
    });
  ...
  it("clicking an existing participant badge triggers removePart", async () => {
    ...
    await waitFor(() => {
      expect(api).toHaveBeenCalledWith("/events/e1/participants/p1", {
        method: "DELETE",
      });
      expect(onChanged).toHaveBeenCalledTimes(1);
    });
  ...
  it("adding a participant via select calls api and resets select", async () => {
    ...
    await waitFor(() => {
      expect(api).toHaveBeenCalledWith("/events/e1/participants/p2", {
        method: "POST",
      });
      expect(onChanged).toHaveBeenCalledTimes(1);
    });
