...
export const EventController = {
  ...
  create: asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!req.body?.title || !req.body?.date || !location) {
      return res.status(400).json({
        error:
          "title, date and a complete address (street, houseNumber, postalCode, city) are required.",
      });
    }
    ...
    try {
      ...
      /* Ignore geocoding failures during creation */
    }
  ...
  byParticipant: asyncHandler(async (req: Request, res: Response) =>
    res.json(
      await EventService.list({ participant: req.params.participantId }),
    ),
  ),
  ...
  weather: asyncHandler(async (req: Request, res: Response) => {
    ...
    if (lat == null || lon == null) {
      ...
      const geo = await geocodeLocation(
        locStr || [zip, cty].filter(Boolean).join(" "),
        zip,
        cty,
      );
      ...
      try {
        await EventService.update(id, { lat, lon });
      } catch {
        /* ignore */
      }
    }
