...
export const ParticipantController = {
  ...
  list: asyncHandler(async (_req: Request, res: Response) =>
    res.json(await ParticipantService.list()),
  ),

  // Get a specific participant by ID
  get: asyncHandler(async (req: Request, res: Response) =>
    res.json(await ParticipantService.get(req.params.id)),
  ),

  // Create a new participant
  create: asyncHandler(async (req: Request, res: Response) =>
    res.status(201).json(await ParticipantService.create(req.body)),
  ),

  // Update an existing participant by ID
  update: asyncHandler(async (req: Request, res: Response) =>
    res.json(await ParticipantService.update(req.params.id, req.body)),
  ),

  // Delete a participant by ID
  remove: asyncHandler(async (req: Request, res: Response) =>
    res.json(await ParticipantService.remove(req.params.id)),
  ),
