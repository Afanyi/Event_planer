...
export async function getForecastSummary(
  ...
  if (dayItems.length === 0) {
    const msg = {
      available: false,
      note: "No forecast for this date (likely beyond 5 days).",
    };
    setCache(key, msg);
    return msg;
  }
  ...
  const temps = dayItems
    .map((i) => i.main.temp as number)
    .filter((n) => Number.isFinite(n));
  const icons = dayItems
    .map((i) => i.weather?.[0]?.icon as string)
    .filter(Boolean);
  const descs = dayItems
    .map((i) => i.weather?.[0]?.description as string)
    .filter(Boolean);
  ...
  const avgPop = Math.round(
    (pop.reduce((a, b) => a + b, 0) / pop.length) * 100,
  );
