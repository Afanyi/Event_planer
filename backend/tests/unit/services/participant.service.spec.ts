...
describe('ParticipantService', () => {
    ...
    test('create() validates and checks uniqueness by name', async () => {
        ...
        (Participant.create as any).mockResolvedValue({ _id: 'p3', name: 'Dana', email: 'd@x.io' });
        const ok = await ParticipantService.create({ name: 'Dana', email: 'd@x.io' });
        expect(ok._id).toBe('p3');

        (Participant.findOne as any).mockResolvedValue({ _id: 'dup' });
        await expect(ParticipantService.create({ name: 'Dana', email: 'd@x.io' }))
            .rejects.toThrow('Participant name already exists.');
    });

    test('update() returns updated or 404', async () => {
        (Participant.findByIdAndUpdate as any).mockResolvedValue({ _id: 'p4', name: 'Neo' });
        const ok = await ParticipantService.update('6562a1c4e9e6f1f1f1f1f1f1', { name: 'Neo' });
        expect(ok.name).toBe('Neo');

        (Participant.findByIdAndUpdate as any).mockResolvedValue(null);
        await expect(ParticipantService.update('6562a1c4e9e6f1f1f1f1f1f1', { name: 'Neo' }))
    ...
    test('remove() returns ok or 404', async () => {
        ...
        await expect(ParticipantService.remove('6562a1c4e9e6f1f1f1f1f1f1'))
            .rejects.toThrow('Participant not found');
    });
});
