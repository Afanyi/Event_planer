...
describe("EventService", () => {
  ...
  test("list() returns array, filters by q/from/to/location", async () => {
    (Event.find as any).mockReturnValue(
      eventChain([{ _id: "e1", title: "Meetup" }]),
    );
  ...
  test("get() returns event or throws 404", async () => {
    ...
    await expect(EventService.get("6562a1c4e9e6f1f1f1f1f1f1")).rejects.toThrow(
      "Event not found",
    );
  });
  ...
  test("remove() returns ok or 404", async () => {
    ...
    await expect(
      EventService.remove("6562a1c4e9e6f1f1f1f1f1f1"),
