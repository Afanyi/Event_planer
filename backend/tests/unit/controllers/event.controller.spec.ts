...
describe("EventController", () => {
  ...
  test("create -> 201", async () => {
    ...
    const req = mockReq(
      {},
      { title: "New", date: "2025-10-10T10:00:00Z", location: "Frankfurt" },
    );
    const res = mockRes();
    const next = mockNext();
  ...
  test("relation routes", async () => {
    ...
    await EventController.addTag(
      mockReq({ id: "e1", tagId: "t1" }) as any,
      res as any,
      next as any,
    );

    EventService.removeTag.mockResolvedValue({ _id: "e1" });
    await EventController.removeTag(
      mockReq({ id: "e1", tagId: "t1" }) as any,
      res as any,
      next as any,
    );
    ...
    await EventController.addParticipant(
      mockReq({ id: "e1", participantId: "p1" }) as any,
      res as any,
      next as any,
    );

    EventService.removeParticipant.mockResolvedValue({ _id: "e1" });
    await EventController.removeParticipant(
      mockReq({ id: "e1", participantId: "p1" }) as any,
      res as any,
      next as any,
    );
