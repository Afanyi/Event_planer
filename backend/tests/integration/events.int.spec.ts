...
describe("Events API", () => {
  ...
  it("filters by tag id and by tag names", async () => {
    ...
    await Event.create([
      {
        title: "Work Only",
        date: iso("2025-10-01T10:00:00Z"),
        tags: [tWork._id],
        location: "Darmstadt",
      },
      {
        title: "Urgent Only",
        date: iso("2025-10-02T10:00:00Z"),
        tags: [tUrg._id],
        location: "Frankfurt",
      },
    ]);
  ...
  it("filters by participant id or by participant name/email", async () => {
    const pAlex = await Participant.create({
      name: "Alex",
      email: "alex@uni.de",
    });
    ...
    await Event.create([
      {
        title: "Alex Event",
        date: iso("2025-10-05T10:00:00Z"),
        participants: [pAlex._id],
        location: "Darmstadt",
      },
      {
        title: "Bob Event",
        date: iso("2025-10-06T10:00:00Z"),
        participants: [pBob._id],
        location: "Frankfurt",
      },
    ]);
    ...
    const byId = await api()
      .get("/api/events")
      .query({ participant: String(pAlex._id) });
  ...
  it("relation endpoints: add/remove tag/participant; by-tag/by-participant", async () => {
    ...
    // add tag
    const addTag = await api().post(
      `/api/events/${id}/tags/${String(tag._id)}`,
    );
