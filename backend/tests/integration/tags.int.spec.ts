...
describe('Tags API', () => {
    it('CRUD flow', async () => {
        // Create
        const create = await api().post('/api/tags').send({ name: 'work', color: '#333333' });
        ...
        // Update
        const upd = await api().put(`/api/tags/${create.body._id}`).send({ color: '#ff0000' });
    ...
    it('rejects duplicate names', async () => {
        ...
        const res = await api().post('/api/tags').send({ name: 'unique', color: '#222' });
